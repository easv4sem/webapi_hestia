...
export class DeviceRepositoryMongoDB implements IDeviceRepository{
    ...
    async deleteDeviceById(id: string): Promise<boolean> {
        const collection = await this.database.getCollectionAsync<IDevice>("devices");
    ...
    async readAllDevices(): Promise<IDevice[]> {
        return await this.database.getCollectionAsync<IDevice>("devices").then(collection => collection.find({}).toArray()) || [];
    }

    async readDeviceById(id: string): Promise<IDevice> {
        return await this.database.getCollectionAsync<IDevice>("devices").then(collection => collection.findOne({PIUniqueIdentifier: id})) || undefined;
    }

    async readDeviceByMacAddress(macAddress: string): Promise<IDevice> {
        return await this.database.getCollectionAsync<IDevice>("devices").then(collection => collection.findOne({Mac: macAddress})) || undefined;
    }
